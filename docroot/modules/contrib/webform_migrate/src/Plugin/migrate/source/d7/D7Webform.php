...
class D7Webform extends DrupalSqlBase implements ImportAwareInterface, RollbackAwareInterface {
  ...
  /**
   * Stores rule that can't be migrated.
   *
   * @var array
   */
  protected array $rulesUnmigrated = [];
  ...
  public function prepareRow(Row $row) {
    ...
    $temp = [];
    foreach ($this->rulesUnmigrated as $k => $d) {
      array_push($temp, $k);
    }
    $row->setSourceProperty('unmigratable_rules', $temp);
  ...
  private function buildFormElements($nid) {
    ...
    $rules_map = [];
    $flag = FALSE;
    if ($this->moduleExists('webform_validation')) {
      $flag = TRUE;

      // Fetch rules.
      $query = $this->select('webform_validation_rule', 'wvr');
      $query->fields('wvr', [
        'ruleid',
        'rulename',
        'nid',
      ...
      foreach ($rules as $rule) {
        $rules_reduce[$rule['ruleid']]['validator'] = $rule['validator'];
        $rules_reduce[$rule['ruleid']]['data'] = $rule['data'];
        $rules_reduce[$rule['ruleid']]['rulename'] = $rule['rulename'];
      }

      // Fetch rule-component relational table.
      $query = $this->select('webform_validation_rule_components', 'wvrc');
      $query->innerJoin('webform_validation_rule', 'wvr', 'wvrc.ruleid = wvr.ruleid');
      $query->condition('nid', $nid);
      $query->fields('wvrc', [
        'ruleid',
        'cid',
      ]);
      $rules_map = $query->execute()->fetchAll();
    }
    ...
    foreach ($components as $component) {
      $component_form_keys[$component['form_key']] = '0';
    ...
    foreach ($elements_tree as $element) {
      ...
      // Build rules.
      if (($rule_config = $this->buildRules($rules_reduce, $rules_map, $element, $xref, $component_form_keys)) && $flag == TRUE) {
        $markup .= "$indent" . $rule_config . "";
      }
  ...
  /**
   * Build rules and translate them to d8|d9 config.
   */
  private function buildRules($rules, $rules_map, $element, $components, $component_form_keys) {

    $rule_stack = [];
    foreach ($rules_map as $map) {
      if ($map['cid'] == $element['cid']) {
        array_push($rule_stack, $map['ruleid']);
      }
    }
    $equal_rule = $component_form_keys;
    $result = [];
    $flag_equal = 0;
    if (!empty($rule_stack)) {
      foreach ($rule_stack as $rule) {
        switch ($rules[$rule]['validator']) {
          case 'equal':
            $flag_equal = 1;
            unset($equal_rule[$element['form_key']]);
            foreach ($rules_map as $map) {
              if ($map['ruleid'] == $rule && $map['cid'] != $element['cid']) {
                $equal_rule[$components[$map['cid']]] = (string) $components[$map['cid']];
              }
            }
            break;

          default:
            $this->rulesUnmigrated[$rules[$rule]['rulename']] = 1;
        }
      }
      if ($flag_equal == 1) {
        $result['equal'] = "  '#equal': true\n";
        $result['equal'] .= "  '#equal_stepwise_validate': 0\n";
        $result['equal'] .= "  '#equal_components':\n";
        foreach ($equal_rule as $key => $value) {
