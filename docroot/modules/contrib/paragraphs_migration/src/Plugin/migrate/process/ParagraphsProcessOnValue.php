 ...
 *   id = "pm_paragraphs_process_on_value"
 * )
 */
class ParagraphsProcessOnValue extends ProcessPluginBase {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    if (empty($this->configuration['source_value'])) {
      throw new \InvalidArgumentException("Required argument 'source_value' not set for pm_paragraphs_process_on_value plugin");
    }
    if (!isset($this->configuration['expected_value'])) {
      throw new \InvalidArgumentException("Required argument 'expected_value' not set for pm_paragraphs_process_on_value plugin");
    }
    if (empty($this->configuration['process']) || !is_array($this->configuration['process'])) {
      throw new \InvalidArgumentException("Required argument 'process' not set or invalid for pm_paragraphs_process_on_value plugin");
    }
    ...
    if ($source_value === $this->configuration['expected_value']) {
      ...
      $source['pm_paragraphs_process_on_value_source_field'] = $value;
      ...
      if (array_key_exists('plugin', $process)) {
        if (empty($process['source'])) {
          $process['source'] = 'pm_paragraphs_process_on_value_source_field';
        }
      }
      else {
        if (empty($process[0]['source'])) {
          $process[0]['source'] = 'pm_paragraphs_process_on_value_source_field';
        }
      }
      $source = $row->getSource();
      $source['pm_paragraphs_process_on_value_source_field'] = $value;
